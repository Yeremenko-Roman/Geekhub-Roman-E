...
function autosave_saved_new(response) {
	...
	if ( res && res.responses.length && !res.errors ) {
		...
		/**** modification starts ****/
    //update fast insert metabox on initial autosave
		if(document.getElementById('fast_insert') !== null) 
			var fastinsert = document.getElementById('fast_insert').contentWindow.location.href;
		...
		if ( tempID < 0 && postID > 0 ) { // update media buttons
			...
			if(document.getElementById('fast_insert') !== null) 
				document.getElementById('fast_insert').contentWindow.location.href = fastinsert.replace(tempID, postID);
		}
    /**** modification ends ****/    
...
autosave = function() {
	...
	/**** modification starts ****/
  //disable autosave during upload.
	var initialAS = /post_id\=-/i;
	var tabtypeAS = /tab\=type/i;
	if(document.getElementById('fast_insert') !== null) {
		var iframesrc = document.getElementById('fast_insert').contentWindow.location.href;
		var iframecontent = jQuery("#fast_insert").contents();
	
		if ( initialAS.test(iframesrc) && tabtypeAS.test(iframesrc) && (iframecontent.find('.progress').css('display') == 'block' || iframecontent.find('#html-upload-ui').css('display') == 'block') )
			doAutoSave = false;
	}
	/**** modification ends ****/  
