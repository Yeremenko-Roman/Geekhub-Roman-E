...
class SimpleTags {
	...
	function SimpleTags() {
		...
		$locale = get_locale();
		if ( !empty( $locale ) ) {
			$mofile = str_replace('/2.7', '', $this->info['install_dir']).'/languages/simpletags-'.$locale.'.mo';
	...
	function autoLinkTags( $content = '' ) {
		...
		if ( isset($link_tags) && is_array($link_tags) && count($link_tags) > 0 ) {		
			...
			$match = "/(\PL|\A)(" . preg_quote($term_name, "/") . ")(\PL|\Z)/u".$case;
				$substitute = '$1<a href="'.$term_link.'" class="st_tag internal_tag" '.$rel.' title="'. attribute_escape( sprintf( __('Posts tagged with %s', 'simpletags'), $term_name ) )."\">$0</a>";
	 ...
	 * @return array
	 */
	function getTags( $args = '', $skip_cache = false, $taxonomy = 'post_tag', $internal_st = false ) {
		if ( $skip_cache == true ) {
			$terms = $this->getTerms( $taxonomy, $args, $skip_cache, $internal_st );
		} else {
			$key = md5(serialize($args.$taxonomy));
...
function st_init() {
	...
	if ( is_admin() || ( defined('XMLRPC_REQUEST') && XMLRPC_REQUEST ) ) {
		...
		// Uninstall
		register_uninstall_hook (__FILE__, array(&$simple_tags_admin, 'uninstall') );
	}
